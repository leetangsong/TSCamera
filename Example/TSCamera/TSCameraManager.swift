...
import TSHandyKit
import AVFoundation
public class TSCameraManager: NSObject {
    ...
    func setupCamera() {
        ...
        guard let firstVideoDevice = devicesVideo.first,
              ...
              let audio = try? AVCaptureDeviceInput.init(device: firstAudioDevice) else {
                  error("初始化相机失败")
                  return
        }
...
extension TSCameraManager: AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate{
...
extension TSCameraManager: CAAnimationDelegate {
...
extension TSKitClassWrapper where Base == UIImage{
    public static func image(fromCameraResource name: String) -> UIImage?{
